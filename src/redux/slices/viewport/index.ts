...
const ZOOM_STEP_DELTA = 53;
const MIN_BOUNDS_WIDTH = 1e-6;
const MIN_BOUNDS_HEIGHT = 1e-6;
...
const viewportSlice = createSlice({
  ...
  reducers: {
    ...
    wheelZoom(
      ...
      const factor = state.zoomStep ** (-deltaY / ZOOM_STEP_DELTA);
      const next = clamp(state.scale * factor, state.minScale, state.maxScale);
      const t = zoomAroundScreenPoint(
        originScreen,
        next,
    ...
    zoomToFit(
      ...
      const bw = Math.max(MIN_BOUNDS_WIDTH, bounds.width);
      const bh = Math.max(MIN_BOUNDS_HEIGHT, bounds.height);
